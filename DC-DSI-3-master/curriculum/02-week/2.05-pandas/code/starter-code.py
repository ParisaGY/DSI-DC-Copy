...
users.age.value_counts('users')        # can also be used with numeric variables
...
import pandas as pd
drinks=pd.read_table('https://raw.githubusercontent.com/josephnelson93/GA-DSI/master/example-lessons/plotting-with-pandas/drinks.csv'
...
drinks.continent.value_counts('drinks') 
...
users.occupation.value_counts('users')[0:3]
...
users_no_header = pd.read_table('https://raw.githubusercontent.com/justmarkham/DAT8/master/data/u.user_original',sep='|', index_col=False)
print(users_no_header)
'''
...
drinks.sort('total_litres_of_pure_alcohol', ascending=False)[0:10]

# BONUS: sort 'users' by 'occupation' and then by 'age' (in a single command)

(users.sort('occupation')).sort('age')
...
drinks = pd.read_csv('https://raw.githubusercontent.com/josephnelson93/GA-DSI/master/example-lessons/plotting-with-pandas/drinks.csv', header=0, names=drink_cols)
print(drinks)
...
drinks = pd.read_csv('https://raw.githubusercontent.com/josephnelson93/GA-DSI/master/example-lessons/plotting-with-pandas/drinks.csv', header=0, names=drink_cols, na_filter=False)
...
ufo= pd.read_csv('https://raw.githubusercontent.com/justmarkham/DAT8/master/data/ufo.csv', header=0, names=ufo_cols)
print(ufo)

ufo.Colors_Reported.value_count('ufo')

# what are the four most frequent colors reported?
ufo.Colors.value_counts('ufo')
users.occupation.value_counts('users')[0:3]
...
ufo['Location'] = ufo.City + ufo.extra +ufo.State
print(ufo)
ufo.drop('extra', 1,inplace=True)
print(ufo)
